...
mrange = ["0100","0200","0300","0400","0500","0600","0700","0800","0900",
          "1000","1100","1200","1300","1400","1500","1600","1700","1800","1900",
          "2000","2500","3000","4000","5000","6000","7000","8000","9000",
          "10000","15000","20000","30000","40000","50000",
          "60000","70000"]
seeds = np.arange(0,5) # 5 different seeds for each point
        ...
        for U in Urange:
            ...
            for m in mrange:
                ...
                if primary not in [16,-16]:
                    output = "%s/%s_light_%s_NC_m4_%s_Um4_%s_seed_%d"%(out_dir,light_generator,primary,m,Ustr,seed)
                    ...
                    tasks.append(f"{primary},{light_generator},light,{N},{output},{experiment},NC,{m},{Ustr},{seed}")

                # charm simulation
                output = "%s/%s_charm_%s_NC_m4_%s_Um4_%s_seed_%d"%(out_dir,charm_generator,primary,m,Ustr,seed)
                # if os.path.exists(output+".parquet"):
                #     continue
                #     #print(f"File {output}.parquet exists, skipping...")
                tasks.append(f"{primary},{charm_generator},charm,{N},{output},{experiment},NC,{m},{Ustr},{seed}")
