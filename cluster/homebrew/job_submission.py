...
for XSModel in ["CC"]:
    ...
    for gen_l,gen_c in zip(light_generators,charm_generators):
        ...
        while Nstart < light_file_length:
            ...
            cmd = "sbatch --export=GEN=%s,MESON=light,Nstart=%s,N=%s,DetModel=%s,XSModel=%s,LakeCenter=%s,LakeExtent=%s submit_simulation.sbatch"%(gen_l,Nstart,N,DetModel,XSModel,LakeCenter,LakeExtent)
            print(cmd)
            os.system(cmd)
            Nstart+=chunk_size
        ...
        while Nstart < charm_file_length:
            ...
            cmd = "sbatch --export=GEN=%s,MESON=charm,Nstart=%s,N=%s,DetModel=%s,XSModel=%s,LakeCenter=%s,LakeExtent=%s submit_simulation.sbatch"%(gen_c,Nstart,N,DetModel,XSModel,LakeCenter,LakeExtent)
            print(cmd)
            os.system(cmd)
            Nstart+=chunk_size